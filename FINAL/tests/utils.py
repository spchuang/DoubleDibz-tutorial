...
class FlaskTestCaseMixin(object):
   ...
   def assertJsonStatusCode(self, response, status_code):
      return self.assertStatusCode(self.assertJson(response), status_code)
      
   def assertJsonLength(self, response, length):
      d = self._jsonLoad(response)
      self.assertEqual(len(d['data']), length)
      
   def _jsonLoad(self, response):
      self.assertOkJson(response)
      return json.loads(response.data)
      
   def assertJsonError(self, response, code, type):
      self.assertJsonStatusCode(response, code)
      self.assertIn('"errors"', response.data)
      self.assertIn('"type": "%s"' % type, response.data)
      
   def assertJsonFormError(self, response):
      self.assertJsonStatusCode(response, 422)
      self.assertIn('"errors"', response.data)
      self.assertIn('"type": "Form validation error"', response.data)
      
   def assertJsonWithData(self, response, data):
      self.assertOkJson(response)
      
      d = self._jsonLoad(response)
      for key in data:
         self.assertEqual(str(d['data'][key]), data[key])
         
   def assertJsonHashtags(self, response, hashtags):
      d = self._jsonLoad(response)
      for tag in d['data']['hashtags']:
         self.assertIn(tag['name'], hashtags)
